...
fn get_platform_info(target: &str) -> Option<(&'static str, &'static str)> {
    match target {
        // Android
        t if t.contains("aarch64") && t.contains("android") => Some(("android-arm64-v8a", "tar.gz")),
        t if t.contains("armv7") && t.contains("android") => Some(("android-armeabi-v7a", "tar.gz")),
        ...
        // iOS
        t if t.contains("aarch64") && t.contains("ios") && !t.contains("sim") => Some(("ios-arm64", "tar.gz")),
        t if t.contains("aarch64") && t.contains("ios") && t.contains("sim") => Some(("ios-simulator-arm64", "tar.gz")),
        t if t.contains("x86_64") && t.contains("ios") => Some(("ios-simulator-x86_64", "tar.gz")),

        // macOS
        t if t.contains("aarch64") && t.contains("apple") && t.contains("darwin") => Some(("macos-arm64", "tar.gz")),
        t if t.contains("x86_64") && t.contains("apple") && t.contains("darwin") => Some(("macos-x86_64", "tar.gz")),

        // Linux
        t if t.contains("x86_64") && t.contains("linux") && !t.contains("android") => Some(("linux-x86_64", "tar.gz")),
        t if t.contains("aarch64") && t.contains("linux") && !t.contains("android") => Some(("linux-aarch64", "tar.gz")),
        t if t.contains("arm") && t.contains("linux") && t.contains("gnueabihf") => Some(("linux-armv7", "tar.gz")),
...
pub fn download_and_extract(target: &str, out_dir: &PathBuf) -> Option<PathBuf> {
    ...
    let base_url = std::env::var("SHERPA_NCNN_RELEASE_URL")
        .unwrap_or_else(|_| {
    ...
    if cache_path.exists() && cache_path.join("lib").exists() {
        println!("cargo:warning=Using cached libraries from {}", cache_path.display());
